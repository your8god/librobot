...
void Robot2D<T>::setMotion(const mathStruct::Point2D<T>& motion)
{
	...
	std::vector<T> B {m_point.x, m_point.y, T(1)};
	...
	m_point = mathStruct::Point2D<T>{C[0], C[1]};
}
...
void Robot3D<T>::setMotion(const mathStruct::Point3D<T>& motion)
{
	...
	std::vector<T> B {m_point.x, m_point.y, m_point.z, T(1)};
	std::vector<T> C(size, T(0));
	...
	m_point = mathStruct::Point3D<T>{C[0], C[1], C[2]};
}
